...
**A local-first, model-agnostic autonomous 3D agent that plans, builds, and inspects Blender scenes.**
...
![Aculptoi-built 3×3×3 cube viewed from an inspection camera](docs/assets/examples/rubiks-cube/result.png)
...
<img src="docs/assets/examples/rubiks-cube/inspection-atlas.png" alt="Twelve-view inspection atlas generated by Aculptoi for the Rubik's Cube run" width="1000">

The final Critic still identified one low-severity appearance issue: “Overall: all cubies
are uniform white; no face colors to distinguish sides.” The focused analysis records the
likely missing per-face coloring and a correction criterion. This demonstrates
...
Models and endpoint URLs are examples only—the core application does not hard-code Qwen,
...
context. Original run artifacts are never modified. `max_discovered_issues` and
`max_issue_analysis_requests` bound the resulting request fan-out. This assumes an
endpoint that accepts OpenAI chat-completions multimodal content, as current
vision-capable llama.cpp server builds do.
