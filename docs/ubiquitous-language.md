...
- Say **construction plan** for the immutable descriptive iteration-level breakdown, **construction item** for one semantic unit, **completion criteria** for the item-level artifact first created by its Actor, **Modeling Step** for one item-scoped mutation, and **execution batch** only when a non-empty action list is submitted to the worker. Use **observation request** for a view-only Actor turn.
