...
from aculptoi.config import BlenderConfig, load_config


def test_config_defaults_are_local_first(tmp_path: Path) -> None:
    config = load_config(tmp_path / "missing.toml")
    assert config.actor.base_url == "http://localhost:8080/v1"
    assert config.vision.base_url == "http://localhost:8081/v1"
